...
# search-jobs-linkedin Workflow (v3)
...
- For each matching row: URL-encode the Query value (percent-encode `"` as `%22`, space as `%20`; `OR` stays as literal text) and construct:
  `https://www.linkedin.com/jobs/search-results/?keywords=<encoded_query>`
- `sub_queries = [<url1>, <url2>, ...]`  (one per matching row, in table order)
...
Read `full_jd_content` from `/tmp/jd-<job_id>.md`. Retrieve `kg_context` and `known_contacts` from the per-job map built in Step 4.
...
<numbered list of all URLs in sub_queries, one per line>
...
**OB1:** `log_search_run(profile_slug=<summary_profile_slug>, query=<comma-joined sub_queries>, pages_fetched=<pages_fetched>, total_results=<total_results>, new_after_dedup=<screened+fetch_failed_count>, screened=<screened>, fit_count=<fit_count>, summary_key=<storage_key>)` → capture returned `search_run_id`
...
- `in_run_seen_ids` is shared across all sub-queries and is never reset between iterations.
