...
**Changes from v5:**
- **New Step 8 — Gate 1 (folder audit).** Runs `scripts/audit-application.sh <folder_slug> --tier=folder` after files are saved. Advisory only: never blocks ingestion. Findings are recorded on the application record so a bad folder is visible at creation rather than at submission.
- **JD completeness** is now checked mechanically (Required-bullet count in `jd-*.md` vs `job-description.md`) instead of relying on a prose-style judgment about whether the capture "reads verbatim". A lossy capture that still reads as prose is now caught.
- **`notes-index.md` no longer carries status fields.** `Status` / `Status Detail` are removed from the template; `js_applications` is the single source of truth for status (this matches what `application-summary` already does — it treats the DB as authoritative and overrides the file). The file keeps its `## OB1 Thought Keys` manifest, which is load-bearing for `application-summary` and `interview-prep` and holds more categories than the two DB columns can.
- **Local-mode contradiction resolved.** v5 said local mode writes `notes.md` (not `notes-index.md`), while `create-application` v5 told local mode to read status from `notes-index.md`. Local mode now reads status from `notes.md`; OB1 reads it from `get_application`.
...
**No status fields.** `notes-index.md` deliberately carries no `**Status:**` or `**Status Detail:**` line. Status lives in `js_applications` and is read with `get_application` — duplicating it here created a dual-write that drifted. The header block is identity and provenance only; the file's real job is the thought manifest.
...
## Step 8 — Gate 1: Folder Audit (all modes)

Run the mechanical folder audit immediately after the files are saved:

```bash
bash "$APP_DIR/scripts/audit-application.sh" <folder_slug> --tier=folder
```

**Advisory only — never block.** This step must not halt a run. An ingest pass creating ten stubs cannot stop because one folder is incomplete. The script's `folder` tier always exits 0 for this reason; treat any FAIL text as a warning.

Handle the output:
- **All PASS** — continue silently. Do not report the audit in the run summary.
- **Any WARN** — append the warning list to the application's `status_detail` (OB1: `update_application_status(application_id, <current status>, <detail + " | Folder audit: " + warnings>)`; local: add to the tracker Status Detail cell) so the gap is visible on the record, and include one line per affected folder in the caller's run summary.

**Why here.** This is the earliest point at which folder completeness can be verified, and the cheapest point at which to fix it. The failure this prevents is a lossy JD capture: a JD whose Required-qualification list was truncated during fetch still reads as natural prose, so a style-based check passes it, and the defect is not discovered until the fit assessment and resume have both been built on incomplete requirements — at which point both must be redone.

**Judgment calls the script does not make.** The script checks structure, not meaning. Still verify by reading: that `jd-*.md` is genuinely the original posting rather than a paraphrase, and that `## Company Research` / `## Company & Market Context` contain real findings rather than placeholder text.

## Outputs

Return: `{folder_slug, application_id, verdict, score, profile_match, folder_audit_warnings}`
