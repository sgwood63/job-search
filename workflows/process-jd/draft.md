...
# process-jd Workflow (DRAFT — promoting toward v4)
...
**Changes from v3:**
- Step 4 OB1 path: `upload_file` calls (steps 3–6) are now made in a single parallel turn instead of serially. `update_application_fields` (formerly Step 4.5) is merged into the same parallel turn since it only requires `application_id`, not upload results.
- Step 5 thought capture: `capture_thought(jd_analysis)` and `capture_thought(fit_assessment)` are now called in a single parallel turn when both are needed.
- Step 6 knowledge graph edges: all `create_knowledge_edge` calls are issued in a single parallel turn instead of a serial loop.

**Changes from v2:**
- Haiku screening output (Step 1) extended with two new fields: `domain_tags` and `jd_requirements_structured`.
- New Step 4.5 (OB1 mode only): call `update_application_fields` to store `domain_tags` and `jd_requirements` on the `js_applications` row. (Now merged into Step 4 parallel turn.)
