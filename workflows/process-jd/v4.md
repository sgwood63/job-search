...
**Changes from v3:**
- New Step 0 (OB1 only): KG context lookup via `get_entity_neighbors` before Haiku screening. Surfaces prior skill signals, existing applications, and known contacts for the company. Result passed to Haiku as `kg_context`.
- Step 4 OB1 path: `upload_file` calls now made in a single parallel turn with `update_application_fields` (merged since it only requires `application_id`, not upload results).
