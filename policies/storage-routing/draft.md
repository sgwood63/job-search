...
**Changes from v3:**
- Upload routing: binary files and text > ~50KB now go directly to REST API — never attempt MCP first (no MCP-then-fallback on size errors).
...
**⚠ Route before calling anything. Binary files and large text must never touch the MCP tool — go directly to REST.**

| File type | Upload method |
|---|---|
| Text files (`.md`, `.json`, `.txt`) ≤ ~50KB | `upload_file` MCP tool |
| Binary files (PDF, images, any non-text content_type) | **REST API via `curl` — NEVER MCP** |
| Text files > ~50KB | **REST API via `curl` — NEVER MCP** |

**MCP tool** — use for small text files only:
```
...
**⚠ NEVER attempt `upload_file` MCP for binary files or text > ~50KB.** Route directly to REST API — no MCP attempt, no size-error fallback. If you encounter an MCP size error for a binary or large file, the routing rule was not followed; do not retry via MCP. Fix the routing and use REST directly.
