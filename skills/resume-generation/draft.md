...
**Changes from v5:**
- **Gate 2 (release audit)** added in two halves: after the resume `.md` upload in Phase 1, and after PDF generation in Phase 2. Blocking — this is the last point before the artifact leaves Claude's control.
- Rationale: `/apply` runs *after* the user has already submitted on the company's portal, so no gate at `/apply` can prevent anything. Handing over the PDF is the release point, so every check must clear before then.
...
1.5. **Gate 2a — release audit (markdown).** Run:

```bash
bash "$APP_DIR/scripts/audit-application.sh" <folder_slug> --tier=release
```

The PDF does not exist yet, so the two PDF checks are expected to FAIL — ignore those. Act on everything else, in particular:
- resume `.md` not matching `<FirstName_LastName>_<Role>.md` — rename before continuing
- a stray `resume.md` — keep the correctly-named file, delete the duplicate

**Fix these now, not later.** Renaming costs nothing here. Once the PDF is generated from a misnamed `.md`, the wrong name is baked into the artifact the user attaches to an application, and the recruiter sees it.
...
8. **Gate 2b — release audit (full).** Re-run the release tier after the PDF is uploaded:

```bash
bash "$APP_DIR/scripts/audit-application.sh" <folder_slug> --tier=release
```

**This gate blocks.** A non-zero exit means the folder is not fit to hand over — resolve every FAIL before telling the user the resume is ready. Do not report completion, and do not surface the PDF, while this gate is failing.

**Why this is the last gate.** The user applies on the company's portal without running any command first; `/apply` is a post-hoc recorder, so nothing downstream can catch a defect. The moment the PDF is handed over it can be submitted — anything not caught here ships.

Alongside the mechanical checks, surface any unresolved decision input the user needs *before* submitting: location or work-authorization constraints, employment type where the posting is ambiguous (an hourly rate, or an application that lands in a staffing ATS, often signals contract rather than FTE), and compensation below the applicant's stated floor. This is the last moment those are actionable.
