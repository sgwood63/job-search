...
---
A dedicated job search assistant app built at `$SOURCE_DIRECTORY/app/`.

**Status**: Complete (2026-04-15). app.py (Streamlit UI), engine.py (API + file engine), config.yaml, and 6 process YAMLs all built. Two-directory architecture: app source (git-tracked) + APPLICANT_DIR (untracked).
...
- `$SOURCE_DIRECTORY/` — app source, git-tracked: process YAMLs, templates/, app-process memory/
- `$APPLICANT_DIR` — applicant data, NOT git-tracked: profiles/, applications/, base-documents/, application-tracker.md, applicant.md, memory/
...
**Path prefix convention**: `app:PATH` → resolved from `$SOURCE_DIRECTORY/`; `applicant:PATH` → resolved from `$APPLICANT_DIR`.
