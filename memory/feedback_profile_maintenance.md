...
- [ ] **B2. Create profile content library** — `profiles/[profile-slug]-CONTENT.md` with: opening summaries (2–3 variants), capabilities section, role bullets by position, portfolio section (if applicable), cover letter guidance. Source all bullets from role-achievements.md and EXPERIENCE-REFERENCE.md only. **Capabilities format:** store as bullet lists under bold category headings — see `feedback_resume_generation.md` for the convention and flatten-on-output rule.
...
- [ ] **B4.5. PROFILES-QUICK-REFERENCE.md search queries** — Add a row to the `## Search Queries` table for the new profile: one OR-query combining role/title terms across all naming conventions. Include adjacent titles: names other companies use for the same function (e.g., "Solutions Architect" alongside "Solutions Engineer"). Aim for 8–14 terms for broad market coverage. Format: `| <profile-slug> | \`"Term A" OR "Term B" OR "Term C"\` |` Also add a row to the `## Location Modes` table in `PROFILES-QUICK-REFERENCE.md` for the new profile. The mode semantics are defined in `workflows/search-jobs/draft.md`.
