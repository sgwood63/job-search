...
Deferred content migrations: /ingest + /ingestLI full workflow extraction; applicant-setup.md as a workflow. Reference artifacts archived in `$APP_DIR/docs/architecture/`: source design PDF, as-approved plan, implementation record, and **hermes-phase-3-4-roadmap.md** — the pickup-able design for the deferred phases (Temporal deployment/worker/workflow detail, js_audit_events DDL, learning-loop mechanics). Start there when resuming phase 3 or 4.
